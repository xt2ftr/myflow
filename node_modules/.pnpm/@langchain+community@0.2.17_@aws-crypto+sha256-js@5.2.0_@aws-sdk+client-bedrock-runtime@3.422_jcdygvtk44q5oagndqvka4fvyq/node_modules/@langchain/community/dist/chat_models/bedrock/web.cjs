...
function formatTools(tools) {
    ...
    if (tools.every((tc) => (0, function_calling_1.isStructuredTool)(tc))) {
        return tools.map((tc) => ({
            name: tc.name,
    ...
    if (tools.every((tc) => (0, base_1.isOpenAITool)(tc))) {
        return tools.map((tc) => ({
    ...
    if (tools.every((tc) => (0, anthropic_js_1.isAnthropicTool)(tc))) {
        return tools;
    }
    if (tools.some((tc) => (0, function_calling_1.isStructuredTool)(tc)) ||
        tools.some((tc) => (0, base_1.isOpenAITool)(tc)) ||
        tools.some((tc) => (0, anthropic_js_1.isAnthropicTool)(tc))) {
...
class BedrockChat extends chat_models_1.BaseChatModel {
    ...
    async *_streamResponseChunks(messages, options, runManager) {
        if (this._anthropicTools) {
            const { generations } = await this._generateNonStreaming(messages, options);
                        ...
                        if (this.usesMessagesApi) {
                            ...
                            if (finalMetrics != null &&
                                ...
                                chunk.message.usage_metadata = {
                                    ...
                                    total_tokens: finalMetrics.inputTokenCount +
