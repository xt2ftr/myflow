...
class SummaryChatHistory extends ChatHistory {
   ...
   */ tokenizer = _GlobalsHelper.globalsHelper.defaultTokenizer.encode;
    tokensToSummarize;
    messages;
    summaryPrompt;
    llm;
    ...
    async summarize() {
        ...
        }while (this.tokenizer(promptMessages[0].content).length > this.tokensToSummarize)
   ...
   */ calcConversationMessages(transformSummary) {
    ...
    async requestMessages(transientMessages) {
        ...
        const tokens = requestMessages.reduce((count, message)=>count + this.tokenizer((0, _utils.extractText)(message.content)).length, 0);
